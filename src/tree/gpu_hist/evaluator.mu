...
void GPUHistEvaluator::Reset(common::HistogramCuts const &cuts, common::Span<FeatureType const> ft,
  ...
  if (cuts.HasCategorical()) {
    ...
    need_sort_histogram_ =
        thrust::any_of(thrust::musa::par(alloc), beg, end, [=] XGBOOST_DEVICE(size_t i) {
          auto idx = i - 1;
    ...
    dh::safe_cuda(
        musaMemsetAsync(split_cats_.data().get(), '\0', split_cats_.size() * sizeof(CatST)));
    ...
    thrust::transform(thrust::musa::par(alloc), it, it + feature_idx_.size(), feature_idx_.begin(),
...
common::Span<bst_feature_t const> GPUHistEvaluator::SortHistogram(
  ...
  thrust::transform(thrust::musa::par(alloc), it, it + data.size(), dh::tbegin(data),
  ...
  thrust::stable_sort_by_key(thrust::musa::par(alloc), dh::tbegin(data), dh::tend(data),
