...
#include "evaluate_splits.muh"
#include "expand_entry.muh"
...
// This kernel uses block_size == warp_size. This is an unusually small block size for a musa kernel
...
// typical musa kernels. Thus the SM can be highly utilised by a small number of threads. It was
...
void GPUHistEvaluator::CopyToHost(const std::vector<bst_node_t> &nidx) {
  ...
  for (auto idx : nidx) {
    ...
    dh::safe_cuda(musaMemcpyAsync(
        ...
        d_cats.GetNodeCatStorage(idx).size_bytes(), musaMemcpyDeviceToHost, copy_stream_.View()));
  }
}
...
void GPUHistEvaluator::EvaluateSplits(Context const *ctx, const std::vector<bst_node_t> &nidx,
  ...
  if (is_column_split_) {
    ...
    dh::safe_cuda(musaMemcpyAsync(current_rank.data(), out_splits.data(),
    ...
    dh::LaunchN(out_splits.size(), ctx->MUSACtx()->Stream(),
  ...
  dh::LaunchN(d_inputs.size(), ctx->MUSACtx()->Stream(), [=] __device__(size_t i) mutable {
...
GPUExpandEntry GPUHistEvaluator::EvaluateSingleSplit(Context const *ctx, EvaluateSplitInputs input,
  ...
  dh::safe_cuda(musaMemcpyAsync(&root_entry, out_entries.data().get(), sizeof(GPUExpandEntry),
