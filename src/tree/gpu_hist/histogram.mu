...
GradientQuantiser::GradientQuantiser(Context const* ctx, common::Span<GradientPair const> gpair,
  ...
  Pair p =
      dh::Reduce(thrust::musa::par(alloc), beg, beg + gpair.size(), Pair{}, thrust::plus<Pair>{});
...
struct HistogramKernel {
  ...
  HistogramKernel(Context const* ctx, FeatureGroupsAccessor const& feature_groups,
    ...
    auto init = [&](auto& kernel) {
      if (this->shared) {
        dh::safe_cuda(musaFuncSetAttribute(kernel, musaFuncAttributeMaxDynamicSharedMemorySize,
                                           max_shared_memory));
      }
      ...
      std::int32_t n_mps = 0;
      dh::safe_cuda(musaDeviceGetAttribute(&n_mps, musaDevAttrMultiProcessorCount, ctx->Ordinal()));

      std::int32_t n_blocks_per_mp = 0;
      dh::safe_cuda(musaOccupancyMaxActiveBlocksPerMultiprocessor(&n_blocks_per_mp, kernel,
...
class DeviceHistogramBuilderImpl {
 ...
 public:
  ...
  void BuildHistogram(MUSAContext const* ctx, EllpackDeviceAccessor const& matrix,
...
void DeviceHistogramBuilder::BuildHistogram(MUSAContext const* ctx,
