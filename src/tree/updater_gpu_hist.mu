...
#include "../common/musa_context.muh"  // CUDAContext
#include "../common/device_helpers.muh"
#include "../common/hist_util.h"
#include "../common/timer.h"
#include "../data/ellpack_page.muh"
#include "../data/ellpack_page.h"
#include "constraints.muh"
#include "driver.h"
#include "gpu_hist/evaluate_splits.muh"
#include "gpu_hist/expand_entry.muh"
#include "gpu_hist/feature_groups.muh"
#include "gpu_hist/gradient_based_sampler.muh"
#include "gpu_hist/histogram.muh"
#include "gpu_hist/row_partitioner.muh"
#include "hist/param.h"
#include "param.h"
#include "updater_gpu_common.muh"
...
struct GPUHistMakerDevice {
 ...
 public:
  ...
  void Reset(HostDeviceVector<GradientPair>* dh_gpair, DMatrix* dmat, int64_t num_columns) {
    ...
    dh::safe_cuda(musaSetDevice(ctx_->Ordinal()));
    ...
    dh::safe_cuda(musaMemcpyAsync(d_gpair.data().get(), dh_gpair->ConstDevicePointer(),
  ...
  void EvaluateSplits(const std::vector<GPUExpandEntry>& candidates, const RegTree& tree,
    ...
    dh::safe_cuda(musaMemcpyAsync(
        d_node_inputs.data().get(), h_node_inputs.data(),
        h_node_inputs.size() * sizeof(EvaluateSplitInputs), musaMemcpyDefault));

    this->evaluator_.EvaluateSplits(ctx_, nidx, max_active_features, dh::ToSpan(d_node_inputs),
                                    shared_inputs, dh::ToSpan(entries));
    dh::safe_cuda(musaMemcpyAsync(pinned_candidates_out.data(),
  ...
  void BuildHist(int nidx) {
    ...
    this->histogram_.BuildHistogram(ctx_->MUSACtx(), page->GetDeviceAccessor(ctx_->Device()),
  ...
  void UpdatePositionColumnSplit(EllpackDeviceAccessor d_matrix,
    ...
    dh::safe_cuda(musaMemcpyAsync(split_data_storage.data().get(), split_data.data(),
                                  num_candidates * sizeof(NodeSplitData), musaMemcpyDefault));
  ...
  void FinalisePosition(RegTree const* p_tree, DMatrix* p_fmat, ObjInfo task,
    ...
    dh::safe_cuda(musaMemcpyAsync(d_nodes.data().get(), p_tree->GetNodes().data(),
  ...
  void FinalisePositionInPage(
    ...
    dh::safe_cuda(musaMemcpyAsync(positions.data().get(), s_position.data(),
                                  s_position.size_bytes(), musaMemcpyDeviceToDevice,
                                  ctx_->MUSACtx()->Stream()));
  ...
  bool UpdatePredictionCache(linalg::MatrixView<float> out_preds_d, RegTree const* p_tree) {
    ...
    CHECK(p_tree);
    CHECK(out_preds_d.Device().IsMUSA());
    ...
    dh::safe_cuda(musaMemcpyAsync(nodes.data().get(), h_nodes.data(),
                                  h_nodes.size() * sizeof(RegTree::Node), musaMemcpyHostToDevice,
                                  ctx_->MUSACtx()->Stream()));
    auto d_nodes = dh::ToSpan(nodes);
    CHECK_EQ(out_preds_d.Shape(1), 1);
    dh::LaunchN(d_position.size(), ctx_->MUSACtx()->Stream(),
  ...
  GPUExpandEntry InitRoot(RegTree* p_tree) {
    ...
    GradientPairInt64 root_sum_quantised =
        dh::Reduce(ctx_->MUSACtx()->CTP(), gpair_it, gpair_it + gpair.size(),
...
class GPUGlobalApproxMaker : public TreeUpdater {
 public:
  ...
  void InitDataOnce(DMatrix* p_fmat) {
    ...
    CHECK(ctx_->IsMUSA()) << error::InvalidMUSAOrdinal();
    uint32_t column_sampling_seed = common::GlobalRandom()();
    this->column_sampler_ = std::make_shared<common::ColumnSampler>(column_sampling_seed);
