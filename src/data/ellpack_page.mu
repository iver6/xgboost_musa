...
#include "device_adapter.muh"              // for NoInfInData
...
void CopyGHistToEllpack(GHistIndexMatrix const& page, common::Span<size_t const> d_row_ptr,
  ...
  dh::LaunchN(row_stride * page.Size(), [=] __device__(size_t idx) mutable {
    ...
    common::musa::DispatchBinType(bin_type, [&](auto t) {
      using T = decltype(t);
...
EllpackPageImpl::EllpackPageImpl(Context const* ctx, GHistIndexMatrix const& page,
  ...
  dh::safe_cuda(musaMemcpyAsync(d_row_ptr.data(), page.row_ptr.data(), d_row_ptr.size_bytes(),
                                musaMemcpyHostToDevice, ctx->MUSACtx()->Stream()));
  ...
  for (size_t gpu_batch = 0; gpu_batch < gpu_nbatches; ++gpu_batch) {
    ...
    if (row_batch.data.DeviceCanRead()) {
      ...
      dh::safe_cuda(musaMemcpyAsync(
          ...
          n_entries * sizeof(Entry), musaMemcpyDefault));
    } else {
