...
#include "../common/musa_pinned_allocator.h"  // for pinned_allocator
#include "../common/device_helpers.muh"       // for CUDAStreamView, DefaultStream
#include "ellpack_page.muh"                   // for EllpackPageImpl
...
struct EllpackHostCache {
  thrust::host_vector<std::int8_t, common::musa::pinned_allocator<std::int8_t>> cache;

  void Resize(std::size_t n, dh::MUSAStreamView stream) {
...
class EllpackHostCacheStreamImpl {
 ...
 public:
  ...
  [[nodiscard]] bst_idx_t Write(void const* ptr, bst_idx_t n_bytes) {
    ...
    dh::safe_cuda(musaMemcpyAsync(cache_->cache.data() + cur_ptr_, ptr, n_bytes, musaMemcpyDefault,
                                  dh::DefaultStream()));
    cur_ptr_ = n;
    return n_bytes;
  }
  ...
  [[nodiscard]] bool Read(void* ptr, bst_idx_t n_bytes) {
    ...
    dh::safe_cuda(musaMemcpyAsync(ptr, cache_->cache.data() + cur_ptr_, n_bytes, musaMemcpyDefault,
                                  dh::DefaultStream()));
    cur_ptr_ += n_bytes;
    return true;
  }
...
void EllpackPageSourceImpl<F>::Fetch() {
  dh::safe_cuda(musaSetDevice(this->Device().ordinal));
  if (!this->ReadCache()) {
