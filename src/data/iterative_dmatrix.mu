...
#include "../common/hist_util.muh"
#include "batch_utils.h"  // for RegenGHist
#include "device_adapter.muh"
#include "ellpack_page.muh"
...
void IterativeDMatrix::InitFromMUSA(Context const* ctx, BatchParam const& p,
  ...
  dh::safe_cuda(musaGetDevice(&current_device));
  auto get_device = [&]() {
    auto d = (ctx->IsMUSA()) ? ctx->Device() : DeviceOrd::MUSA(current_device);
    CHECK(!d.IsCPU());
    return d;
  };
  ...
  do {
    ...
    dh::safe_cuda(musaSetDevice(get_device().ordinal));
    if (cols == 0) {
      cols = num_cols();
    ...
    if (!ref) {
      ...
      musa_impl::Dispatch(proxy, [&](auto const& value) {
    ...
    row_stride = std::max(row_stride, musa_impl::Dispatch(proxy, [=](auto const& value) {
                            return GetRowCounts(value, row_counts_span, get_device(), missing);
                          }));
    nnz += thrust::reduce(thrust::musa::par(alloc), row_counts.begin(), row_counts.end());
    batches++;
  } while (iter.Next());
  iter.Reset();
  ...
  while (iter.Next()) {
    ...
    auto new_impl = musa_impl::Dispatch(proxy, [&](auto const& value) {
  ...
  if (!ellpack_) {
    ...
    if (ctx->IsMUSA()) {
      ...
      *ellpack_->Impl() =
      ...
      // for cut reference.
      auto musa_ctx = ctx->MakeMUSA();
      this->Info().feature_types.SetDevice(musa_ctx.Device());
      *ellpack_->Impl() =
          EllpackPageImpl(&musa_ctx, *this->ghist_, this->Info().feature_types.ConstDeviceSpan());
    }
  }
  CHECK(ellpack_);
