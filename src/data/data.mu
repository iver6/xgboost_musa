 ...
 */
#include "../common/musa_context.muh"
#include "../common/device_helpers.muh"
#include "../common/linalg_op.muh"
...
auto SetDeviceToPtr(void const* ptr) {
  ...
  dh::safe_cuda(musaPointerGetAttributes(&attr, ptr));
  int32_t ptr_device = attr.device;
  dh::safe_cuda(musaSetDevice(ptr_device));
  return ptr_device;
}

template <typename T, int32_t D>
void CopyTensorInfoImpl(MUSAContext const* ctx, Json arr_interface, linalg::Tensor<T, D>* p_out) {
  ArrayInterface<D> array(arr_interface);
  if (array.n == 0) {
    p_out->SetDevice(DeviceOrd::MUSA(0));
    p_out->Reshape(array.shape);
    return;
  }
  CHECK_EQ(array.valid.Capacity(), 0)
      << "Meta info like label or weight can not have missing value.";
  auto ptr_device = DeviceOrd::MUSA(SetDeviceToPtr(array.data));
  ...
  if (array.is_contiguous && array.type == ToDType<T>::kType) {
    p_out->ModifyInplace([&](HostDeviceVector<T>* data, common::Span<size_t, D> shape) {
      ...
      // set data
      data->Resize(array.n);
      dh::safe_cuda(musaMemcpyAsync(data->DevicePointer(), array.data, array.n * sizeof(T),
                                    musaMemcpyDefault, ctx->Stream()));
    });
    return;
  }
  p_out->Reshape(array.shape);
...
void CopyQidImpl(ArrayInterface<1> array_interface, std::vector<bst_group_t>* p_group_ptr) {
  ...
  auto d = DeviceOrd::MUSA(SetDeviceToPtr(array_interface.data));
  ...
  bool non_dec = true;
  dh::safe_cuda(musaMemcpy(&non_dec, flag.data().get(), sizeof(bool),
  ...
  thrust::inclusive_scan(thrust::musa::par(alloc), cnt.begin(),
...
void MetaInfo::SetInfoFromMUSA(Context const& ctx, StringView key, Json array) {
  // multi-dim float info
  if (key == "base_margin") {
    CopyTensorInfoImpl(ctx.MUSACtx(), array, &base_margin_);
    return;
  } else if (key == "label") {
    CopyTensorInfoImpl(ctx.MUSACtx(), array, &labels);
  ...
  CopyTensorInfoImpl(ctx.MUSACtx(), array, &t);
  if (key == "weight") {
    ...
    auto valid =
        thrust::none_of(ctx.MUSACtx()->CTP(), d_feature_weights.data(),
