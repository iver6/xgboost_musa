 ...
 * \file simple_dmatrix.mu
 */
#include <thrust/copy.h>

#include "device_adapter.muh"  // for CurrentDevice
#include "simple_dmatrix.muh"
...
SimpleDMatrix::SimpleDMatrix(AdapterT* adapter, float missing, std::int32_t nthread,
  ...
  auto device = (!adapter->Device().IsMUSA() || adapter->NumRows() == 0)
                    ? DeviceOrd::MUSA(dh::CurrentDevice())
                    : adapter->Device();
  CHECK(device.IsMUSA());
