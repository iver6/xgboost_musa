...
#include "../common/device_helpers.muh"  // for DefaultStream, CUDAEvent
#include "array_interface.h"
#include "xgboost/logging.h"

namespace xgboost {
void ArrayInterfaceHandler::SyncMUSAStream(std::int64_t stream) {
  switch (stream) {
    case 0:
      /**
    ...
    default: {
      dh::MUSAEvent e;
      e.Record(dh::MUSAStreamView{reinterpret_cast<musaStream_t>(stream)});
      dh::DefaultStream().Wait(e);
    }
  }
}

bool ArrayInterfaceHandler::IsMUSAPtr(void const* ptr) {
  if (!ptr) {
    return false;
  }
  ...
  if (err == musaErrorInvalidValue) {
    ...
    // other errors, `musaErrorNoDevice`, `musaErrorInsufficientDriver` etc.
    return false;
  }
}
}  // namespace xgboost
