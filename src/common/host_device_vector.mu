...
#include "device_vector.muh"  // for DeviceUVector
...
// the handler to call instead of musaSetDevice; only used for testing
static void (*musaSetDeviceHandler)(int) = nullptr;  // NOLINT

void SetMusaSetDeviceHandler(void (*handler)(int)) {
  musaSetDeviceHandler = handler;
}
 ...
 public:
  ...
  void Extend(HostDeviceVectorImpl* other) {
    ...
    if (HostCanWrite() && other->HostCanRead()) {
      ...
      dh::safe_cuda(musaMemcpyAsync(this->DevicePointer() + ori_size, ptr,
                                    other->Size() * sizeof(T), musaMemcpyDeviceToDevice,
                                    dh::DefaultStream()));
    }
  }
  ...
  void SetDevice(DeviceOrd device) {
    ...
    if (device_.IsMUSA() && device.IsMUSA()) {
  ...
  auto Resize(std::size_t new_size, U&&... args) {
    ...
    if ((Size() == 0 && device_.IsMUSA()) || (DeviceCanWrite() && device_.IsMUSA())) {
  ...
  void LazySyncHost(GPUAccess access) {
    ...
    SetDevice();
    dh::safe_cuda(musaMemcpy(data_h_.data(), thrust::raw_pointer_cast(data_d_->data()),
                             data_d_->size() * sizeof(T), musaMemcpyDeviceToHost));
  }
  ...
  void LazySyncDevice(GPUAccess access) {
    ...
    LazyResizeDevice(data_h_.size());
    SetDevice();
    dh::safe_cuda(musaMemcpyAsync(thrust::raw_pointer_cast(data_d_->data()), data_h_.data(),
                                  data_d_->size() * sizeof(T), musaMemcpyHostToDevice,
                                  dh::DefaultStream()));
    gpu_access_ = access;
  }
 ...
 private:
  ...
  void CopyToDevice(HostDeviceVectorImpl* other) {
    if (other->HostCanWrite()) {
      ...
      gpu_access_ = GPUAccess::kWrite;
      SetDevice();
      dh::safe_cuda(musaMemcpyAsync(thrust::raw_pointer_cast(data_d_->data()),
                                    thrust::raw_pointer_cast(other->data_d_->data()),
                                    data_d_->size() * sizeof(T), musaMemcpyDefault,
                                    dh::DefaultStream()));
    }
  }
  ...
  void CopyToDevice(const T* begin) {
    ...
    gpu_access_ = GPUAccess::kWrite;
    SetDevice();
    dh::safe_cuda(musaMemcpyAsync(thrust::raw_pointer_cast(data_d_->data()), begin,
                                  data_d_->size() * sizeof(T), musaMemcpyDefault,
                                  dh::DefaultStream()));
  }
  ...
  void SetDevice() {
    ...
    if (musaSetDeviceHandler == nullptr) {
      dh::safe_cuda(musaSetDevice(device_.ordinal));
    } else {
      (*musaSetDeviceHandler)(device_.ordinal);
    }
