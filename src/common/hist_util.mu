...
void SortByWeight(dh::device_vector<float>* weights, dh::device_vector<Entry>* sorted_entries) {
  ...
  thrust::sort_by_key(thrust::musa::par(alloc), sorted_entries->begin(), sorted_entries->end(),
                      weights->begin(), detail::EntryCompareOp());

  // Scan weights
  dh::XGBCachingDeviceAllocator<char> caching;
  thrust::inclusive_scan_by_key(
      thrust::musa::par(caching), sorted_entries->begin(), sorted_entries->end(), weights->begin(),
      weights->begin(),
...
[[nodiscard]] Span<float const> UnifyWeight(MUSAContext const* cuctx, MetaInfo const& info,
  ...
  if (!info.weights_.Empty()) {
    ...
    dh::safe_cuda(musaMemcpyAsync(d_weight_out.data(), hessian.data(), hessian.size_bytes(),
                                  musaMemcpyDefault));
  }
  return d_weight_out;
}
