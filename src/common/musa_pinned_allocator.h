...
class pinned_allocator {
 public:
  ...
  inline pointer allocate(size_type cnt, const_pointer = nullptr) { // NOLINT
    ...
    dh::safe_cuda(musaMallocHost(reinterpret_cast<void**>(&result), cnt * sizeof(value_type)));
    return result;
  }

  inline void deallocate(pointer p, size_type) { dh::safe_cuda(musaFreeHost(p)); } // NOLINT
