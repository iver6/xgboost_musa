...
void CopyTo(Span<T> out, Span<U> src) {
  ...
  dh::safe_cuda(musaMemcpyAsync(out.data(), src.data(),
                                out.size_bytes(),
                                musaMemcpyDefault));
}
...
common::Span<thrust::tuple<uint64_t, uint64_t>> MergePath(
  ...
  thrust::merge_by_key(
      thrust::musa::par(alloc), x_merge_key_it, x_merge_key_it + d_x.size(),
  ...
  thrust::transform(
      thrust::musa::par(alloc), transform_it, transform_it + x_ptr.size(),
      out_ptr.data(),
  ...
  thrust::exclusive_scan_by_key(
      thrust::musa::par(alloc), scan_key_it, scan_key_it + merge_path.size(),
...
void MergeImpl(DeviceOrd device, Span<SketchEntry const> const &d_x,
  ...
  dh::safe_cuda(musaSetDevice(device.ordinal));
...
size_t SketchContainer::ScanInput(Span<SketchEntry> entries, Span<OffsetT> d_columns_ptr_in) {
  ...
  thrust::inclusive_scan_by_key(
      thrust::musa::par(alloc), key_it, key_it + entries.size(),
      val_it, val_it,
      thrust::equal_to<size_t>{},
...
void SketchContainer::MakeCuts(Context const* ctx, HistogramCuts* p_cuts, bool is_column_split) {
  ...
  if (has_categorical_) {
    ...
    auto new_end = thrust::reduce_by_key(
        thrust::musa::par(alloc), key_it, key_it + in_cut_values.size(), val_it, d_max_keys.begin(),
    ...
    dh::caching_device_vector<SketchEntry> d_max_results(d_in_columns_ptr.size() - 1,
                                                         default_entry);
    thrust::scatter(thrust::musa::par(alloc), d_max_values.begin(), d_max_values.end(),
